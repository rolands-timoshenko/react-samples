...
import QUERY from "./TurbotActivityActiveGrantActionMenu.queries";
        ...
        breadcrumbs: [
          ...
          },
          {
            ...
            filters: data.grant.identity.type.trunk.items.map(item => {
              return {
                label: item.title || item.turbot.id,
                key: "resourceType",
                value: item.turbot.id
              };
            })
...
class TurbotActivityActiveGrantActionMenuContainer extends Component {
  static propTypes = {
...
export default enhance(TurbotActivityActiveGrantActionMenuContainer);
